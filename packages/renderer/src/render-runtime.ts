...
function isCommandVisible(mvpMatrix: Mat4): boolean {
  const ndcCenter = transformPoint(mvpMatrix, vec3(0, 0, 0));
  return (
    ndcCenter.x >= -1.25 &&
    ndcCenter.x <= 1.25 &&
    ndcCenter.y >= -1.25 &&
...
class RendererImpl implements Renderer {
  ...
  public render(scene: Scene, camera: Camera): void {
    ...
    this.lastFrame = Object.freeze({
      ...
      culledEntityCount,
      materialSwitchCount: countSwitches(commands, (command) => command.materialId),
      meshSwitchCount: countSwitches(commands, (command) => command.meshId),
      commands,
    });
  }
